...
1. Create a bcrypt password-file entry for the `student` account.
   ...
   ![Evidence 1](Evidence/1.%20Create%20a%20password%20file.png)
...
3. Create an Nginx configuration that protects `/` with Basic authentication and uses `/etc/nginx/.htpasswd` as the credential store.
   ...
   ![Evidence 2](Evidence/2.%20The%20configuration%20was%20created%20as%20default.conf.png)
...
5. Run Nginx, bind host port `8080` to port `80`, and mount both configuration files read-only from the working directory.
   ...
   ![Evidence 3](Evidence/3.%20bind%20host%20port%208080%20to%20port%2080.png)
...
7. Test access without credentials.
   ...
   **Result:** `no-creds: 401`. The server rejected the unauthenticated request.  
   ...
   ![Evidence 4](Evidence/4.%20Test%20access%20without%20credentials..png)
...
9. Test access with the valid lab credentials.
   ...
   **Result:** `Authenticated OK`, which corresponds to HTTP 200.
   ...
   ![Evidence 5](Evidence/5.%20Test%20access%20with%20the%20valid%20lab%20credentials.png)
...
1. Generate a random Base32 shared secret and display it for enrolment in an authenticator app.
   ...
   ![Evidence 6](Evidence/6.%20Create%20a%20shared%20secret%20(base32)%20and%20generate%20the%20current%206-digit%20code.png)
...
5. Validate a code the user types.

   Valid and invalid codes were verified.
   
   ![Evidence 7](Evidence/7.%20Validate%20a%20code%20the%20user%20types.png)  
...
1. Create the Kind cluster, the `app` namespace, and the `dev` service account.
   ...
   ![Evidence 8](Evidence/8.%20create%20cluster.png)
...
3. Create a Role restricted to `get` and `list` verbs on `pods`, then bind it to the service account.
   ...
   ![Evidence 9](Evidence/9.%20Developer%20may%20only%20read%20pods.png)
...
5. Test the permissions as the service account.
   ...
   ![Evidence 10](Evidence/10.%20Test%20the%20permissions%20as%20the%20service%20account..png)
...
1. Create isolated frontend and backend Docker networks.
   ...
   ![Evidence 11](Evidence/11.%20Create%20two%20segmented%20networks.png)
...
3. Attach the database only to `backend-net`; attach the application to `backend-net` and then `frontend-net`; attach the web container only to `frontend-net`.
   ...
   ![Evidence 12](Evidence/12.%20DB%20only%20on%20backend-net;%20app%20on%20both;%20web%20only%20on%20frontend-net.png)
   ![Evidence 12.1](Evidence/12.1%20DB%20only%20on%20backend-net;%20app%20on%20both;%20web%20only%20on%20frontend-net.png)
...
5. Test the prohibited and permitted frontend-to-database connections.
   ...
   **Expected result:** `web` to `db` fails (`BLOCKED`), but `app` to `db` succeeds.
   ...
   ![Evidence 13](Evidence/13.%20web%20-%20db%20should%20FAIL%20and%20app%20-db%20should%20WORK.png)
...
1. Run a temporary Alpine container with `NET_ADMIN` so it can manipulate iptables. Install iptables, set the INPUT policy to `DROP`, permit TCP/443, permit loopback traffic, and list the resulting rules.
...
2. The displayed ruleset had **INPUT policy `DROP`**, plus an `ACCEPT` rule for TCP destination port 443 and an `ACCEPT` rule for all loopback traffic.
   ...
   ![Evidence 14](Evidence/14.%20Inside%20a%20throwaway%20container%20with%20iptables.png)
...
1. Start the Nginx service with a non-root user, read-only root filesystem, no Linux capabilities, no privilege escalation, and a temporary writable `/tmp`.
   ...
   ![Evidence 15](Evidence/15.%20A%20hardened%20run%20of%20a%20service.png)
   ![Evidence 15.1](Evidence/15.1%20A%20hardened%20run%20of%20a%20service.png)
...
3. Inspect the configuration.
   ...
   **Observed result:** `User=1000:1000` and `ReadOnly=true`  
   ...
   ![Evidence 16](Evidence/16.%20docker%20inspect%20hardened.png)
...
5. Scan the Nginx Alpine image for known high and critical vulnerabilities.
   ...
   ![Evidence 17.1](Evidence/17.1%20Scan%20an%20image%20for%20known%20vulnerabilities.png)  
   ![Evidence 17.2](Evidence/17.2%20Scan%20an%20image%20for%20known%20vulnerabilities.png)  
...
The lab environment was cleaned up with the following commands, as captured:
...
![Evidence 18](Evidence/18.%20Cleanup%20&%20Teardown.png)  
...
| 6–7 | TOTP secret/code generation, failed validation, and successful MFA validation |
| 8–10 | Kind/namespace/service-account creation, RBAC role/binding, and `can-i` results |
| 11–13 | Docker networks, three-tier deployment, and successful `app → db` connectivity |
| 14 | Default-deny iptables ruleset |
| 15–16 | Hardened container launch and non-root/read-only inspection |
| 17 | Trivy download/progress and clean scan summary |
| 18 | Cleanup of containers, networks, and Kind cluster |
