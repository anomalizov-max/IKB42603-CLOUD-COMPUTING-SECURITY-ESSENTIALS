...
1. Created the sensitive plaintext record:
   ...
   ![1. Create a sample sensitive record.png](Evidence/1.%20Create%20a%20sample%20sensitive%20record.png)
...
3. Encrypted the record using AES-256-CBC with PBKDF2 key derivation and a random salt:
   ...
   ![2. Encrypt with AES-256.png](Evidence/2.%20Encrypt%20with%20AES-256.png)
...
5. Displayed the encrypted file:
   ...
   ![3. Prove it is unreadable.png](Evidence/3.%20Prove%20it%20is%20unreadable.png)
...
7. Decrypted the ciphertext using the same passphrase:
   ...
   ![4. Decrypt back.png](Evidence/4.%20Decrypt%20back.png)
...
1. Generated a 2048-bit RSA private key:
   ...
   ![5. Generate a 2048-bit key pair.png](Evidence/5.%20Generate%20a%202048-bit%20key%20pair.png)
...
5. Encrypted the record with the public key:
   ...
   ![6. Encrypt with the PUBLIC key, decrypt with the PRIVATE key.png](Evidence/6.%20Encrypt%20with%20the%20PUBLIC%20key,%20decrypt%20with%20the%20PRIVATE%20key.png)
...
9. Signed the original record with the private key, using SHA-256:
   ...
   ![7. Sign with the PRIVATE key; verify with the PUBLIC key.png](Evidence/7.%20Sign%20with%20the%20PRIVATE%20key;%20verify%20with%20the%20PUBLIC%20key.png).
...
1. Generated a self-signed RSA certificate for `localhost`, valid for seven days:
   ...
   The certificate-generation command was completed before the HTTPS container was started. Evidence:

   ![8. Generate a self-signed certificate.png](Evidence/8.%20Generate%20a%20self-signed%20certificate.png)
...
2. Started an Nginx container, exposed HTTPS on port 8443, and mounted the certificate, private key, and test record:
   ...
   ![9. Serve HTTPS on port 8443 using a small container.png](Evidence/9.%20Serve%20HTTPS%20on%20port%208443%20using%20a%20small%20container.png)
...
4. Retrieved the record over HTTPS, accepting the self-signed certificate for this lab:
   ...
   ![10. Connect over TLS.png](Evidence/10.%20Connect%20over%20TLS.png)
...
1. Configured the LocalStack endpoint:
   ...
   ![11. Create a customer master key (CMK) and capture its KeyId.png](Evidence/11.%20Create%20a%20customer%20master%20key%20(CMK)%20and%20capture%20its%20KeyId.png)
...
3. Created the tenant-A customer-managed KMS key:
   ...
   ![12. Copy the KeyId from the output into KEY_A below.png](Evidence/12.%20Copy%20the%20KeyId%20from%20the%20output%20into%20KEY_A%20below.png)  
...
5. Used the tenant-A KMS key to encrypt the small plaintext `hello`:
   ...
   ![13. Encrypt a small secret directly with KMS.png](Evidence/13.%20Encrypt%20a%20small%20secret%20directly%20with%20KMS.png)
...
1. Requested a 256-bit data key from KMS:
   ...
   ![14. Ask KMS for a data key.png](Evidence/14.%20Ask%20KMS%20for%20a%20data%20key.png)  
...
3. Saved the plaintext value as `datakey.b64` and the wrapped value as `datakey.enc`, then decoded the plaintext data key locally:
   ...
   ![15. Encrypt the big file locally with the PLAINTEXT data key.png](Evidence/15.%20Encrypt%20the%20big%20file%20locally%20with%20the%20PLAINTEXT%20data%20key.png)  
...
7. Destroyed the on-disk plaintext key material:
   ...
   ![16. Destroy the plaintext data key from disk.png](Evidence/16.%20Destroy%20the%20plaintext%20data%20key%20from%20disk.png)  
...
1. Created a second customer-managed key for tenant B:
   ...
   ![17. A separate key for tenant B.png](Evidence/17.%20A%20separate%20key%20for%20tenant%20B.png)  
...
3. Scheduled tenant A's key for deletion with the minimum seven-day window:
   ...
   ![18. Schedule deletion of tenant A's key.png](Evidence/18.%20Schedule%20deletion%20of%20tenant%20A's%20key.png)  
...
5. The evidence then cancels the deletion and disables the tenant-A key:
   ...
   ![19. Disable it immediately to simulate erasure.png](Evidence/19.%20Disable%20it%20immediately%20to%20simulate%20erasure.png)  
...
7. Attempted to decrypt the wrapped data key:
   ...
   ![20. Attempt to unwrap tenant A's data key now.png](Evidence/20.%20Attempt%20to%20unwrap%20tenant%20A's%20data%20key%20now.png)  
...
1. Calculated the SHA-256 digest of the original record:
   ...
   ![21. Integrity & Tamper-Evidence.png](Evidence/21.%20Integrity%20&%20Tamper-Evidence.png)  
